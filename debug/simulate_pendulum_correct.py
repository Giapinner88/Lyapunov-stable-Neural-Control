...
def main():
    ...
    # BALANCE ONLY: ROA nhỏ (đúng như huấn luyện) + mạnh để đưa vào
    roa_small_theta, roa_small_omega = 0.30, 0.70      # Original small ROA
    roa_exit_theta, roa_exit_omega = 0.55, 1.30        # Thoát ROA nếu quá xa
    ...
    print(f"Bắt đầu mô phỏng BALANCE ONLY (30s)...")
    print(f"  θ ban đầu (norm): {init_theta_norm:.4f} rad ({np.degrees(init_theta_norm):.1f}°)")
    print(f"  ω ban đầu: 0.0 rad/s")
    print(f"  ROA nhỏ (NN): |θ|<{roa_small_theta}, |ω|<{roa_small_omega}")
    print(f"  Chiến lược: PD control mạnh → vào ROA nhỏ → NN controller stabilize")
    print()
    ...
    while data.time < 30.0:
        ...
        if data.time - last_ctrl_time >= control_dt - 1e-6:
            ...
            # Hysteresis ROA (nhỏ)
            in_roa_enter = abs(theta_norm) < roa_small_theta and abs(theta_dot) < roa_small_omega
            ...
            with torch.no_grad():
                if in_stabilization_mode:
                    # NN Output Feedback - được huấn luyện cho ROA nhỏ này
                    ...
                    # BALANCE ONLY: PD control mạnh để ĐƯA VÀO ROA
                    # Mục tiêu: đưa con lắc vào ROA nhỏ để NN tiếp quản
                    k_p = 2.0    # Tăng lên cao hơn để kéo mạnh vào upright
                    k_d = 0.7    # Damping để giảm overshoot
                    
                    u_p = -k_p * theta_norm
                    u_d = -k_d * theta_dot
                    
                    tau_applied = u_p + u_d
                    tau_applied = np.clip(tau_applied, -3.0, 3.0)  # Giới hạn cao để có đủ lực
    ...
    print(f"  PD được dùng: {len(log['in_roa'])-sum(log['in_roa'])} bước ({100*(1-sum(log['in_roa'])/len(log['in_roa'])):.1f}%)")
