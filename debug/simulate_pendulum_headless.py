...
def main():
    ...
    # qpos[0] = 0 là cân bằng dưới, qpos[0] = π là cân bằng trên
    data.qpos[0] = np.pi + 0.01  # Bắt đầu RẤT gần upright (1 độ từ π)
    data.qvel[0] = 0.0
        ...
        if data.time - last_ctrl_time >= control_dt - 1e-6:
            ...
            with torch.no_grad():
                if in_stabilization_mode:
                    ...
                    # BALANCE ONLY: Simple PD control (không swing-up), nhưng mạnh hơn
                    k_p = 1.5  # Tăng từ 0.5 -> 1.5 (anti-position mạnh hơn)
                    k_d = 0.5  # Tăng từ 0.3 -> 0.5 (damping mạnh hơn)
                    u_p = -k_p * theta_norm
                    u_d = -k_d * theta_dot
                    tau_applied = u_p + u_d
                    tau_applied = np.clip(tau_applied, -3.0, 3.0)  # Tăng lên 3.0 để có đủ lực
