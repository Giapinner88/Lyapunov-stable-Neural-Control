...
@hydra.main(version_base=None, config_path="./config", config_name="state_feedback")
def main(cfg: DictConfig):
    ...
    if cfg.model.lyapunov.quadratic:
        ...
        controller.load_state_dict(
            torch.load(
                str(PROJECT_ROOT / cfg.model.controller_path)
            )
        )
    else:
        lyapunov_nn = lyapunov.NeuralNetworkLyapunov(
    ...
    # approximate_controller(controller_target, controller, 2, limit, 0, 0, "output/pendulum_controller.pth", batch_size=10000, max_iter=500)
    # approximate_controller(lyapunov_target, lyapunov_nn, 2, limit, 0, 0, "output/pendulum_lyapunov.pth", batch_size=10000, max_iter=500)
    ...
    if cfg.model.load_lyaloss is not None:
        load_lyaloss = str(PROJECT_ROOT / cfg.model.load_lyaloss)
